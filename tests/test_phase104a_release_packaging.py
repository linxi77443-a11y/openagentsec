#!/usr/bin/env python3
"""
tests/test_phase104a_release_packaging.py — Automated Pytest Suite for Phase-104A-RELEASE-002 (v4.0 Master).
Path: tests/test_phase104a_release_packaging.py
...
from scripts.validate_phase104a_release_package import compute_sha256
...
class TestReleasePackageDeliverables:
    ...
    def test_all_deliverables_exist(self):
        deliverables = [
            ROOT / "docs/release_notes_v4_0.md",
            ROOT / "docs/enterprise_ai_security_platform_v4_0_architecture.md",
            ROOT / "docs/milestone_4_0_safety_and_compliance_charter.md",
            ROOT / "release_v4_0_manifest.yaml",
            ROOT / "checksums_v4_0.sha256",
            ROOT / "scripts/validate_phase104a_release_package.py",
            ROOT / "tests/test_phase104a_release_packaging.py",
            ROOT / "phase104a_release002_execution_summary.yaml",
            ROOT / "delivery.json",
        ]
    ...
    def test_release_notes_v40_content(self):
        rn = ROOT / "docs/release_notes_v4_0.md"
        content = rn.read_text(encoding="utf-8")
        assert "企业级 AI 安全评估平台 v4.0 Master 终局发布说明" in content
        assert "七大核心架构支柱终局交付" in content
        assert "全系统 50 模块最终清单索引" in content
        assert "20 份红队行动报告全量审计收口清单" in content
        assert "终局安全边界声明" in content
        assert "v4.0-FINAL" in content

    def test_architecture_whitepaper_content(self):
        arch = ROOT / "docs/enterprise_ai_security_platform_v4_0_architecture.md"
...
class TestReleaseManifest:
    ...
    def manifest_data(self) -> Dict:
        path = ROOT / "release_v4_0_manifest.yaml"
        with open(path, "r", encoding="utf-8") as f:
            return yaml.safe_load(f)

    def test_manifest_metadata(self, manifest_data):
        meta = manifest_data.get("release_metadata", {})
        assert meta.get("release_id") == "REL-4.0-20260819"
        assert meta.get("release_version") == "v4.0"
        assert meta.get("release_status") == "RELEASE_SEALED"
        assert meta.get("task_id") == "Phase-104A-RELEASE-002"
    ...
    def test_manifest_pillars_all_pass(self, manifest_data):
        ...
        assert pillars["pillar_4_propagation_dynamics"]["status"] == "PASS"
        assert pillars["pillar_5_gatekeeper_8node"]["status"] == "PASS"
        assert pillars["pillar_6_dashboard_and_reports"]["status"] == "PASS"
        assert pillars["pillar_7_known_bad_defenses"]["status"] == "PASS"
        assert pillars["pillar_7_known_bad_defenses"]["interception_rate"] == 1.0
    ...
    def test_manifest_60_extended_scenarios_catalog(self, manifest_data):
...
class TestChecksumsIntegrity:
    """Test SHA-256 integrity against checksums_v4_0.sha256."""
...
class TestReleaseValidatorExecution:
    ...
    def test_validator_script_executes_pass(self):
        from scripts.validate_phase104a_release_package import main as validator_main
        ret = validator_main()
        assert ret == 0
