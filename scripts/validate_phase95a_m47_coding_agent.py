#!/usr/bin/env python3
"""Phase 95A — M47 Coding Agent Command and Credential Boundary Validator.
...
def main():
    global checks_passed, checks_failed
    print("=" * 60)
    print("Phase 95A — M47 Coding Agent Command and Credential Boundary Validation")
    print("=" * 60)
    ...
    print("\n1. Playbook")
    playbook_paths = [
        ROOT / "adversarial_playbooks/m47_coding_agent_command_credential_mvp/m47_adversarial_playbook.yaml",
        ROOT / "adversarial_playbooks/m47_coding_agent_command_credential_mvp/playbook.yaml",
    ]
    ...
    for pb_path in playbook_paths:
        ...
        if playbook:
            ...
            check(len(control_cases) >= 2, f"{pb_path.name} has >= 2 control cases ({len(control_cases)})")

            # Check placeholders <SIM_CMD...> and <SIM_CRED...>
            all_text = yaml.dump(entries)
            sim_cmd_pattern = re.findall(r'<SIM_CMD[\w_]*>', all_text)
            sim_cred_pattern = re.findall(r'<SIM_CRED[\w_]*>', all_text)
            check(len(sim_cmd_pattern) >= 5,
                  f"{pb_path.name} entries contain <SIM_CMD...> placeholders ({len(sim_cmd_pattern)} found)")
            check(len(sim_cred_pattern) >= 3,
                  f"{pb_path.name} entries contain <SIM_CRED...> placeholders ({len(sim_cred_pattern)} found)")

            # Check metadata
            check(meta.get("module_id") == "M47", f"{pb_path.name} metadata module_id == M47")
            ...
            check(meta.get("attacker_type") == "malicious_insider",
                  f"{pb_path.name} metadata attacker_type == malicious_insider")
            check(meta.get("attack_objective") == "role_escalation",
                  f"{pb_path.name} metadata attack_objective == role_escalation")
            ...
            check_security_fields(meta, "M47", f"{pb_path.name} metadata")

    # ================================================================
    # 2. Run Config
    # ================================================================
    print("\n2. Run Config")
    rc_path = ROOT / "run_configs/phase95a_m47_coding_agent_run_config.yaml"
    ...
    if rc:
        ...
        check(rcfg.get("module_id") == "M47", "Run config module_id == M47")
        check(rcfg.get("assessment_mode") == "adversarial_validation", "Run config assessment_mode == adversarial_validation")
        check(rcfg.get("attacker_type") == "malicious_insider", "Run config attacker_type == malicious_insider")
        check(rcfg.get("attack_objective") == "role_escalation", "Run config attack_objective == role_escalation")
        ...
        check_security_fields(rcfg, "M47", "Run config")
    ...
    print("\n3. Execution Results")
    exec_dirs = [
        ROOT / "adversarial_playbooks/m47_coding_agent_command_credential_mvp",
        ROOT / "executions/phase95a_m47_coding_agent",
    ]
    ...
    for edir in exec_dirs:
        ...
        if eres:
            entry_list = eres.get("results", eres) if isinstance(eres, dict) else eres
            check(len(entry_list) >= 14, f"{edir.name}/execution_results has >= 14 entries ({len(entry_list)})")
            for r in entry_list:
                eid = r.get("entry_id", "UNKNOWN")
                check(r.get("real_command_executed") is False, f"{eid} real_command_executed == false")
                check(r.get("real_credentials_read") is False, f"{eid} real_credentials_read == false")
    ...
    print("\n4. Result YAML")
    for edir in exec_dirs:
        for rfile in ["m47_result.yaml", "result.yaml"]:
            rpath = edir / rfile
            if not rpath.exists():
                continue
            ...
            if ryaml:
                check(ryaml.get("module_id") == "M47", f"{edir.name}/{rfile} module_id == M47")
                check(ryaml.get("assessment_mode") == "adversarial_validation", f"{edir.name}/{rfile} assessment_mode == adversarial_validation")
                check(ryaml.get("attacker_type") == "malicious_insider", f"{edir.name}/{rfile} attacker_type == malicious_insider")
                check(ryaml.get("attack_objective") == "role_escalation", f"{edir.name}/{rfile} attack_objective == role_escalation")
                ...
                check(rs.get("exploit_chain_candidate") is False, f"{edir.name}/{rfile} results_summary.exploit_chain_candidate == false")
                check("evidence_trace" in ryaml, f"{edir.name}/{rfile} contains evidence_trace")
                check_security_fields(ryaml, "M47", f"{edir.name}/{rfile}")
    ...
    for edir in exec_dirs:
        ...
        if sc:
            ...
            check(sm.get("module_id") == "M47", f"{edir.name}/scorecard module_id == M47")
            ...
            check_security_fields(sm, "M47", f"{edir.name}/scorecard metadata")
    ...
    # ================================================================
    print("\n7. Documentation")
    doc_path = ROOT / "docs/phase95a_m47_coding_agent_notes.md"
    check(doc_path.exists(), f"Notes document exists at {doc_path}")
    if doc_path.exists():
        doc_text = doc_path.read_text(encoding="utf-8")
        check("malicious_insider" in doc_text, "Notes document mentions malicious_insider")
        check("role_escalation" in doc_text, "Notes document mentions role_escalation")
        ...
        check("<SIM_CMD" in doc_text, "Notes document contains <SIM_CMD placeholders")
        check("<SIM_CRED" in doc_text, "Notes document contains <SIM_CRED placeholders")
    ...
    if checks_failed > 0:
        ...
        print("Phase 95A — M47 Coding Agent Command and Credential Boundary: ALL CHECKS PASSED")
    print("=" * 60)
