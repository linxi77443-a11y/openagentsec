...
Post-process script: reads Phase 56A execution_results.json, fans out each
tool trace entry to all modules listed in modules_under_test, re-runs parser
to generate per-module results + consolidated scorecard.

Does NOT re-call API. Does NOT modify original execution_results.json.
"""
...
CONFIG_PATH = ROOT / "capability_engine/configs/phase56a_simulated_tool_trace_run.yaml"

def main():
    os.makedirs(OUT_DIR, exist_ok=True)

    # 1. Read original execution results (read-only)
    ...
    # 3. Fan out: for each tool trace entry, create copies for all modules_under_test
    fanned_results = []
    fan_out_log = []
    ...
    for r in original_results:
        ...
        if not corpus_entry or not r.get("is_tooltrace", False):
            # Non-tooltrace entries pass through unchanged
        ...
        # Fan out to each module in modules_under_test
        first = True
        for mid in mods:
            mid_lower = mid.lower()
    ...
    # 4. Save fanned-out results to output dir
    out_results_path = OUT_DIR / "execution_results.json"
    ...
    # 5. Run parser on fanned-out results
    ...
    modules, scorecard = parser.parse(str(out_results_path), str(OUT_DIR), run_config)

    # 6. Build consolidated tool_trace_result.yaml
    tt_summary = {"phase": "phase56a1", "description": "Tool Trace Attribution Alignment — fanned-out per-module results"}
    ...
    print(f"Phase 56A.1 attribution alignment complete")
    return True
