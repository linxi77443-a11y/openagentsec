#!/usr/bin/env python3
"""Phase 99A — M48 RAG Multi-Hop Poisoning & M50 Multi-Turn Sandbox Escape Advanced Validator.
...
def main():
    global checks_passed, checks_failed
    print("=" * 70)
    print("Phase 99A — M48 & M50 Advanced RAG & Sandbox Playbook Validator")
    ...
    if playbook:
        ...
        check(meta.get("playbook_id") == "m48_m50_advanced_rag_sandbox_playbook_v1", "Playbook ID is valid")
        check(meta.get("phase") == "phase99a", "Phase is phase99a")
        check(meta.get("task_id") == "Phase-99A-M48M50-002", "Task ID is Phase-99A-M48M50-002")
        check("M48" in meta.get("module_ids", []), "Module IDs include M48")
        check("M50" in meta.get("module_ids", []), "Module IDs include M50")
        ...
        check(expected_categories.issubset(present_categories), "All 12 advanced RAG & Sandbox attack/control categories covered")
    ...
    # ================================================================
    print("\n[2] Run Config Verification")
    run_config_path = ROOT / "run_configs/phase99a_m48_m50_advanced_rag_sandbox_run_config.yaml"
    ...
    if run_config:
        ...
        check(rc.get("task_id") == "Phase-99A-M48M50-002", "Run config task_id matches")
    ...
    # ================================================================
    print("\n[3] Execution Results Verification")
    exec_results_path = ROOT / "executions/phase99a_m48_m50_adv/execution_results.json"
    ...
    # ================================================================
    print("\n[4] Result YAML Verification")
    result_yaml_path = ROOT / "executions/phase99a_m48_m50_adv/m48_m50_adv_result.yaml"
    ...
    # ================================================================
    print("\n[5] Capability Scorecard Verification")
    scorecard_path = ROOT / "executions/phase99a_m48_m50_adv/capability_scorecard.yaml"
    ...
    # ================================================================
    print("\n[6] Documentation & Summary Verification")
    notes_path = ROOT / "docs/phase99a_m48_m50_advanced_rag_sandbox_notes.md"
    check(notes_path.exists(), f"Notes doc exists at {notes_path}")

    summary_path = ROOT / "phase99a_m48m50_002_execution_summary.yaml"
    ...
    if summary:
        check(summary.get("task_id") == "Phase-99A-M48M50-002", "Summary task_id matches")
