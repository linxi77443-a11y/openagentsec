#!/usr/bin/env python3
"""Phase 94A — M05 Output Boundary / Unsafe Conclusion Control MVP Validator.
...
def main():
    global checks_passed, checks_failed
    print("=" * 60)
    print("Phase 94A — M05 Output Boundary / Unsafe Conclusion Control MVP Validation")
    print("=" * 60)
    ...
    # ================================================================
    print("\n1. Corpus")
    corpus_path = ROOT / "adversarial_playbooks/m05_output_boundary_unsafe_conclusion_mvp/playbook.yaml"
    ...
    if corpus:
        ...
        check("unsafe_medical_advice" in categories, "Corpus includes unsafe_medical_advice")
        check("unsafe_legal_opinion" in categories, "Corpus includes unsafe_legal_opinion")
        check("unsafe_financial_advice" in categories, "Corpus includes unsafe_financial_advice")
        check("unsafe_security_conclusion" in categories, "Corpus includes unsafe_security_conclusion")
        check("general_knowledge_query" in categories, "Corpus includes general_knowledge_query")
        ...
        # Module metadata
        check(meta.get("module_id") == "M05", "Corpus module_id == M05")
        ...
        check(meta.get("real_medical_system_allowed") is False,
              "Corpus real_medical_system_allowed == false")

    # ================================================================
    # 2. Run config
    # ================================================================
    print("\n2. Run config")
    rc_path = ROOT / "run_configs/phase94a_m05_output_boundary_unsafe_conclusion_run_config.yaml"
    rc = yaml_load(rc_path)
    check(rc is not None, "Run config YAML loaded")
    if rc:
        rcfg = rc.get("run_config", {})
        check(rcfg.get("phase") == "phase94a", "Run config phase == phase94a")
        check(rcfg.get("module_id") == "M05", "Run config module_id == M05")
    ...
    # ================================================================
    print("\n3. Execution results")
    exec_path = ROOT / "executions/phase94a_m05_mvp/execution_results.json"
    ...
    if exec_results:
        ...
        for r in exec_results:
            ...
            check(r.get("real_medical_system_accessed") is False,
                  f"{rid} real_medical_system_accessed == false")
    ...
    # ================================================================
    print("\n4. Result YAML")
    result_path = ROOT / "executions/phase94a_m05_mvp/m05_result.yaml"
    result_yaml = yaml_load(result_path)
    check(result_yaml is not None, "m05_result.yaml exists")
    if result_yaml:
        check(result_yaml.get("module_id") == "M05", "result module_id == M05")
        ...
        check("unsafe_conclusion_blocked_count" in result_yaml,
              "result has unsafe_conclusion_blocked_count")
        check("disclaimer_provided_count" in result_yaml,
              "result has disclaimer_provided_count")
        check("professional_referral_count" in result_yaml,
              "result has professional_referral_count")
        check("domain_boundary_preserved_count" in result_yaml,
              "result has domain_boundary_preserved_count")
        check("safe_summary_allowed_count" in result_yaml,
              "result has safe_summary_allowed_count")
    ...
    # ================================================================
    print("\n5. Scorecard")
    scorecard_path = ROOT / "executions/phase94a_m05_mvp/capability_scorecard.yaml"
    ...
    if scorecard:
        ...
        check(sm.get("module_id") == "M05", "scorecard module_id == M05")
        ...
        check("unsafe_conclusion_blocked" in rs,
              "scorecard result_summary has unsafe_conclusion_blocked")
        check("disclaimer_provided" in rs,
              "scorecard result_summary has disclaimer_provided")
        check("professional_referral" in rs,
              "scorecard result_summary has professional_referral")
        check("domain_boundary_preserved" in rs,
              "scorecard result_summary has domain_boundary_preserved")
        check("safe_summary_allowed" in rs,
              "scorecard result_summary has safe_summary_allowed")
    ...
    print("\n" + "=" * 60)
    if checks_failed == 0:
        print("Phase 94A M05 Output Boundary / Unsafe Conclusion Control: ALL CHECKS PASSED")
    else:
        print("Phase 94A M05 Output Boundary / Unsafe Conclusion Control: FAILED")
