#!/usr/bin/env python3
"""Phase 91A — M49 Permission-Aware Retrieval Hardening Validator.
...
def main():
    global checks_passed, checks_failed
    print("=" * 60)
    print("Phase 91A — M49 RAG Permission-Aware Retrieval Validation MVP Validation")
    print("=" * 60)
    ...
    # ================================================================
    print("\n1. Corpus")
    corpus_path = ROOT / "adversarial_playbooks/m49_permission_aware_retrieval_hardening/playbook.yaml"
    ...
    if corpus:
        ...
        check("low_priv_to_high_priv_retrieval" in categories,
              "Corpus includes low_priv_to_high_priv_retrieval")
        check("cross_tenant_retrieval_attempt" in categories,
              "Corpus includes cross_tenant_retrieval_attempt")
        check("permission_inheritance_bypass" in categories,
              "Corpus includes permission_inheritance_bypass")
        check("missing_permission_annotation" in categories,
              "Corpus includes missing_permission_annotation")
        check("standard_permission_retrieval" in categories,
              "Corpus includes standard_permission_retrieval")
    ...
    # ================================================================
    print("\n2. Run config")
    rc_path = ROOT / "run_configs/phase91a_m49_permission_aware_retrieval_hardening_run_config.yaml"
    rc = yaml_load(rc_path)
    check(rc is not None, "Run config YAML loaded")
    if rc:
        rcfg = rc.get("run_config", {})
        check(rcfg.get("phase") == "phase91a", "Run config phase == phase91a")
    ...
    # ================================================================
    print("\n3. Execution results")
    exec_path = ROOT / "executions/phase91a_m49_hardening/execution_results.json"
    ...
    if exec_results:
        ...
        for r in exec_results:
            ...
            check(r.get("real_rag_system_accessed") is False,
                  f"{rid} real_rag_system_accessed == false")
    ...
    # ================================================================
    print("\n4. Result YAML")
    result_path = ROOT / "executions/phase91a_m49_hardening/m49_hardening_result.yaml"
    result_yaml = yaml_load(result_path)
    check(result_yaml is not None, "m49_hardening_result.yaml exists")
    if result_yaml:
        ...
        check("permission_aware_retrieval_count" in result_yaml,
              "result has permission_aware_retrieval_count")
        check("cross_permission_retrieval_blocked_count" in result_yaml,
              "result has cross_permission_retrieval_blocked_count")
        check("cross_tenant_retrieval_blocked_count" in result_yaml,
              "result has cross_tenant_retrieval_blocked_count")
        check("permission_inheritance_preserved_count" in result_yaml,
              "result has permission_inheritance_preserved_count")
        ...
        check(result_yaml.get("real_rag_system_accessed") is False,
              "result real_rag_system_accessed == false")

    # ================================================================
    # 5. Scorecard
    # ================================================================
    print("\n5. Scorecard")
    scorecard_path = ROOT / "executions/phase91a_m49_hardening/capability_scorecard_hardening.yaml"
    scorecard = yaml_load(scorecard_path)
    check(scorecard is not None, "capability_scorecard_hardening.yaml exists")
    if scorecard:
        ...
        check("permission_aware_retrieval" in rs,
              "scorecard result_summary has permission_aware_retrieval")
        check("cross_permission_retrieval_blocked" in rs,
              "scorecard result_summary has cross_permission_retrieval_blocked")
        check("cross_tenant_retrieval_blocked" in rs,
              "scorecard result_summary has cross_tenant_retrieval_blocked")
        check("permission_inheritance_preserved" in rs,
              "scorecard result_summary has permission_inheritance_preserved")
    ...
    # ================================================================
    print("\n6. Notes")
    notes_path = ROOT / "docs/phase91a_m49_permission_aware_retrieval_hardening_notes.md"
    notes = file_exists(notes_path, "Notes")
    if notes:
        notes_text = notes_path.read_text()
        check("不连接真实 RAG" in notes_text or "no real rag" in notes_text.lower()
              or "real_rag_system" in notes_text.lower(),
    ...
    if reg:
        ...
        m09 = next((m for m in modules if m.get("module_id") == "M49"), None)
        check(m09 is not None, "M49 exists in registry")
        if m09:
            check(m09.get("production_safety") == "out_of_scope",
                  "Registry M49 production_safety == out_of_scope")
            check(m09.get("confirmed_vulnerability_allowed") is False,
    ...
    deliverables = {
        ...
        "m49_hardening_result.yaml": result_yaml,
        "capability_scorecard_hardening.yaml": scorecard,
    }
    ...
    print("\n" + "=" * 60)
    if checks_failed == 0:
        print("Phase 91A M49 RAG Permission-Aware Retrieval Validation: ALL CHECKS PASSED")
    else:
        print("Phase 91A M49 RAG Permission-Aware Retrieval Validation: FAILED")
