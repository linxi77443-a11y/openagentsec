#!/usr/bin/env python3
"""Phase 111A — M30 Model Behavior Drift Monitoring MVP Validator.
...
def main():
    global checks_passed, checks_failed
    print("=" * 60)
    print("Phase 111A — M30 Model Behavior Drift Monitoring MVP Validation")
    print("=" * 60)
    ...
    # ================================================================
    print("\n1. Corpus")
    corpus_path = ROOT / "adversarial_playbooks/m30_model_behavior_drift_mvp/playbook.yaml"
    ...
    if corpus:
        ...
        check("model_behavior_drift_undetected" in categories,
              "Corpus includes model_behavior_drift_undetected")
        check("drift_threshold_misconfiguration" in categories,
              "Corpus includes drift_threshold_misconfiguration")
        check("drift_source_localization_failure" in categories,
              "Corpus includes drift_source_localization_failure")
        check("drift_impact_assessment_bias" in categories,
              "Corpus includes drift_impact_assessment_bias")
        check("model_version_drift_exploitation" in categories,
              "Corpus includes model_version_drift_exploitation")
        check("prompt_drift_detection_failure" in categories,
              "Corpus includes prompt_drift_detection_failure")
        check("output_distribution_drift" in categories,
              "Corpus includes output_distribution_drift")
        check("safety_guardrail_drift" in categories,
              "Corpus includes safety_guardrail_drift")
        check("control_standard_drift_monitoring" in categories,
              "Corpus includes control_standard_drift_monitoring")
        check("control_normal_model_version_update" in categories,
              "Corpus includes control_normal_model_version_update")
        ...
        # Module metadata
        check(meta.get("module_id") == "M30", "Corpus module_id == M30")
        ...
        check(meta.get("real_model_allowed") is False,
              "Corpus real_model_allowed == false")
        check(meta.get("real_monitoring_system_allowed") is False,
              "Corpus real_monitoring_system_allowed == false")
        check(meta.get("real_drift_detection_algorithm_allowed") is False,
              "Corpus real_drift_detection_algorithm_allowed == false")

    # ================================================================
    # 2. Run config
    # ================================================================
    print("\n2. Run config")
    rc_path = ROOT / "run_configs/phase111a_m30_model_behavior_drift_run_config.yaml"
    rc = yaml_load(rc_path)
    check(rc is not None, "Run config YAML loaded")
    if rc:
        rcfg = rc.get("run_config", {})
        check(rcfg.get("phase") == "phase111a", "Run config phase == phase111a")
        check(rcfg.get("module_id") == "M30", "Run config module_id == M30")
    ...
    # ================================================================
    print("\n3. Execution results")
    exec_path = ROOT / "executions/phase111a_m30_mvp/execution_results.json"
    ...
    if exec_results:
        ...
        for r in exec_results:
            ...
            check(r.get("real_model_accessed") is False,
                  f"{rid} real_model_accessed == false")
            check(r.get("real_monitoring_system_accessed") is False,
                  f"{rid} real_monitoring_system_accessed == false")
            check(r.get("real_drift_detection_executed") is False,
                  f"{rid} real_drift_detection_executed == false")
    ...
    # ================================================================
    print("\n4. Result YAML")
    result_path = ROOT / "executions/phase111a_m30_mvp/m30_result.yaml"
    result_yaml = yaml_load(result_path)
    check(result_yaml is not None, "m30_result.yaml exists")
    if result_yaml:
        check(result_yaml.get("module_id") == "M30", "result module_id == M30")
        ...
        check("model_behavior_drift_count" in result_yaml,
              "result has model_behavior_drift_count")
        check("drift_threshold_misconfiguration_count" in result_yaml,
              "result has drift_threshold_misconfiguration_count")
        check("drift_source_localization_count" in result_yaml,
              "result has drift_source_localization_count")
        check("drift_impact_bias_count" in result_yaml,
              "result has drift_impact_bias_count")
        check("version_drift_count" in result_yaml,
              "result has version_drift_count")
        check("prompt_drift_count" in result_yaml,
              "result has prompt_drift_count")
        check("output_distribution_drift_count" in result_yaml,
              "result has output_distribution_drift_count")
        check("safety_guardrail_drift_count" in result_yaml,
              "result has safety_guardrail_drift_count")
    ...
    # ================================================================
    print("\n5. Scorecard")
    scorecard_path = ROOT / "executions/phase111a_m30_mvp/capability_scorecard.yaml"
    ...
    if scorecard:
        ...
        check(sm.get("module_id") == "M30", "scorecard module_id == M30")
        ...
        check("model_behavior_drift_detected" in rs,
              "scorecard result_summary has model_behavior_drift_detected")
        check("drift_threshold_misconfiguration_detected" in rs,
              "scorecard result_summary has drift_threshold_misconfiguration_detected")
        check("drift_source_localization_detected" in rs,
              "scorecard result_summary has drift_source_localization_detected")
        check("drift_impact_bias_detected" in rs,
              "scorecard result_summary has drift_impact_bias_detected")
        check("version_drift_detected" in rs,
              "scorecard result_summary has version_drift_detected")
        check("prompt_drift_detected" in rs,
              "scorecard result_summary has prompt_drift_detected")
        check("output_distribution_drift_detected" in rs,
              "scorecard result_summary has output_distribution_drift_detected")
        check("safety_guardrail_drift_detected" in rs,
              "scorecard result_summary has safety_guardrail_drift_detected")
    ...
    # ================================================================
    print("\n6. Notes")
    notes_path = ROOT / "docs/phase111a_m30_model_behavior_drift_notes.md"
    check(notes_path.exists(), "Notes file exists")
    if notes_path.exists():
        notes_text = notes_path.read_text()
        check("M30" in notes_text, "Notes reference M30")
        check("Drift" in notes_text or "drift" in notes_text,
              "Notes reference Drift or drift")
    ...
    if registry:
        ...
        m30 = None
        for m in modules:
            if m.get("module_id") == "M30":
                m30 = m
                break
        check(m30 is not None, "M30 entry found in registry")
        if m30:
            check(m30.get("current_status") == "mvp_complete",
                  f"M30 current_status == mvp_complete (got: {m30.get('current_status')})")
            cov = m30.get("coverage", {})
            check(cov.get("coverage_status") == "mvp_complete",
                  f"M30 coverage_status == mvp_complete (got: {cov.get('coverage_status')})")
            check(cov.get("implementation_status") == "mvp_done",
                  f"M30 implementation_status == mvp_done (got: {cov.get('implementation_status')})")
    ...
    print("\n" + "=" * 60)
    if checks_failed == 0:
        print("Phase 111A M30 Model Behavior Drift Monitoring: ALL CHECKS PASSED")
    else:
        print("Phase 111A M30 Model Behavior Drift Monitoring: FAILED")
