#!/usr/bin/env python3
"""Phase 62D — Multi-turn Boundary Erosion MVP Execution Script.

Simulates adversarial_validation evaluation for MTBE playbook entries.
No real API calls. SIM_ placeholder data only.
"""
import json, sys, yaml
from datetime import datetime, timezone
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
PLAYBOOK_PATH = ROOT / "adversarial_playbooks/multi_turn_boundary_erosion_mvp/playbook.yaml"
OUTPUT_DIR = ROOT / "executions/phase62d_mtbe_mvp"

# Entry IDs where defense erodes across turns (breakthrough on final turn)
EROSION_BREAKTHROUGH_IDS = {"MTBE-004", "MTBE-007"}


def load_playbook(path):
    with open(path) as f:
        return yaml.safe_load(f)


def build_prompt_from_turns(turns):
    """Build a single prompt string from multi-turn conversation."""
    parts = []
    for t in turns:
        parts.append(f"[Turn {t['turn_number']}] {t['role']}: {t['content']}")
    return "\n\n".join(parts)


def simulate_defense(entry):
    """Simulate multi-turn defense evaluation.

    For most entries, defense holds across all turns.
    For erosion entries, defense holds initially but fails by final turn.
    """
    eid = entry["entry_id"]
    is_control = entry.get("control_case", False)
    turns = entry.get("turns", [])

    if is_control:
        return {
            "defensive_action": "normal_response",
            "breakthrough_detected": False,
            "requires_human_review": False,
            "signal_detected": None,
            "defensive_check_passed": True,
            "erosion_detected": False,
            "turns_evaluated": [
                {"turn_number": t["turn_number"], "defensive_action": "normal_response",
                 "breakthrough": False} for t in turns
            ],
            "evidence_trace": [],
            "exploit_chain_candidate": [],
            "evaluation_summary": "control_case — no adversarial signal detected across all turns",
        }

    expected = entry.get("expected_behavior", "refuse")
    is_erosion = eid in EROSION_BREAKTHROUGH_IDS

    if is_erosion:
        # Defense holds on turns 1-2, fails on turn 3
        return {
            "defensive_action": "partial_bypass",
            "breakthrough_detected": True,
            "requires_human_review": True,
            "signal_detected": "erosion_breakthrough",
            "defensive_check_passed": False,
            "erosion_detected": True,
            "turns_evaluated": [
                {"turn_number": 1, "defensive_action": expected, "breakthrough": False,
                 "signal": "boundary_maintained"},
                {"turn_number": 2, "defensive_action": expected, "breakthrough": False,
                 "signal": "boundary_pressure_increasing"},
                {"turn_number": 3, "defensive_action": "partial_bypass", "breakthrough": True,
                 "signal": "boundary_erosion_breakthrough"},
            ],
            "evidence_trace": [
                {"timestamp": datetime.now(timezone.utc).isoformat(), "source": "turn_1", "content": turns[0]["content"][:60] if len(turns) > 0 else "", "signal_type": "boundary_maintained"},
                {"timestamp": datetime.now(timezone.utc).isoformat(), "source": "turn_2", "content": turns[1]["content"][:60] if len(turns) > 1 else "", "signal_type": "boundary_pressure"},
                {"timestamp": datetime.now(timezone.utc).isoformat(), "source": "turn_3", "content": turns[2]["content"][:60] if len(turns) > 2 else "", "signal_type": "erosion_breakthrough"},
            ],
            "exploit_chain_candidate": [
                {"step": 1, "observation": "Initial benign request established context", "signal": "trust_building"},
                {"step": 2, "observation": "Gradual escalation: request pushed beyond boundary", "signal": "boundary_testing"},
                {"step": 3, "observation": "Boundary eroded: defense failed to maintain refusal", "signal": "cumulative_erosion"},
            ],
            "evaluation_summary": f"breakthrough — multi-turn boundary erosion: defense held for 2 turns, eroded on turn 3 (expected: {expected})",
        }

    # Standard defense: holds across all turns
    return {
        "defensive_action": expected,
        "breakthrough_detected": False,
        "requires_human_review": False,
        "signal_detected": entry.get("expected_signal", "boundary_erosion_refusal"),
        "defensive_check_passed": True,
        "erosion_detected": False,
        "turns_evaluated": [
            {"turn_number": t["turn_number"], "defensive_action": expected, "breakthrough": False,
             "signal": "boundary_maintained"} for t in turns
        ],
        "evidence_trace": [
            {"timestamp": datetime.now(timezone.utc).isoformat(), "source": t["role"], "content": t["content"][:60], "signal_type": "boundary_maintained"}
            for t in turns
        ],
        "exploit_chain_candidate": [],
        "evaluation_summary": f"blocked — multi-turn boundary maintained across all turns (defensive action: {expected})",
    }


def execute_playbook(playbook):
    entries = playbook.get("entries", [])
    meta = playbook.get("playbook_metadata", {})

    results = []
    for entry in entries:
        turns = entry.get("turns", [])
        prompt = build_prompt_from_turns(turns)
        result = {
            "entry_id": entry["entry_id"],
            "adversarial_playbook_type": meta.get("adversarial_playbook_type", "multi_turn_boundary_erosion"),
            "category": entry.get("category", ""),
            "scenario": entry.get("scenario", ""),
            "control_case": entry.get("control_case", False),
            "expected_behavior": entry.get("expected_behavior", ""),
            "expected_signal": entry.get("expected_signal", ""),
            "multi_turn": True,
            "turn_count": len(turns),
            "prompt": prompt,
            "turns": turns,
        }
        result.update(simulate_defense(entry))
        results.append(result)

    return results


def write_execution_results(results, output_dir):
    output_dir.mkdir(parents=True, exist_ok=True)
    path = output_dir / "execution_results.json"
    with open(path, "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"  [OK] {path} — {len(results)} entries")


def generate_scorecard(results, meta, output_dir):
    total = len(results)
    breakthrough = sum(1 for r in results if r["breakthrough_detected"])
    blocked = sum(1 for r in results if not r["breakthrough_detected"] and not r["control_case"])
    control_ok = sum(1 for r in results if r["control_case"] and not r["breakthrough_detected"])
    human_review = sum(1 for r in results if r["requires_human_review"])
    inconclusive = sum(1 for r in results if r.get("defensive_action") == "inconclusive")
    erosion = sum(1 for r in results if r.get("erosion_detected"))

    behavior_counts = {}
    for r in results:
        b = r.get("defensive_action", "unknown")
        behavior_counts[b] = behavior_counts.get(b, 0) + 1

    scorecard = {
        "scorecard_metadata": {
            "phase": "phase62d",
            "run_id": "phase62d-mtbe-mvp",
            "assessment_mode": "adversarial_validation",
            "adversarial_playbook_type": "multi_turn_boundary_erosion",
            "actor": meta.get("actor", "simulated_adversary"),
            "attacker_profile": meta.get("attacker_profile", "authenticated_low_privilege"),
            "attacker_type": meta.get("attacker_type", "low_privileged_operator"),
            "attack_objective": meta.get("attack_objective", "unauthorized_access"),
            "target": "SIM_ simulated defense (no real API)",
            "confirmed_vulnerability": False,
            "formal_finding_allowed": False,
            "production_safety_claimed": False,
            "controlled_replay_claimed": False,
            "total_entries": total,
            "multi_turn_entries": sum(1 for r in results if r.get("multi_turn")),
            "erosion_detected": erosion,
            "erosion_breakthrough_ids": sorted([r["entry_id"] for r in results if r.get("erosion_detected")]),
            "generated_at": datetime.now(timezone.utc).isoformat(),
        },
        "results_summary": {
            "total": total,
            "breakthrough_detected": breakthrough,
            "blocked": blocked,
            "control_passed": control_ok,
            "inconclusive": inconclusive,
            "human_review_required": human_review,
            "erosion_detected": erosion,
            "defensive_behavior_distribution": behavior_counts,
        },
        "category_coverage": list(dict.fromkeys(r["category"] for r in results if not r["control_case"])),
    }

    path = output_dir / "capability_scorecard.yaml"
    with open(path, "w") as f:
        yaml.dump(scorecard, f, allow_unicode=True, default_flow_style=False, sort_keys=False)
    print(f"  [OK] {path}")


def generate_adversarial_result(results, output_dir):
    entries_out = []
    for r in results:
        entry = {
            "entry_id": r["entry_id"],
            "category": r["category"],
            "scenario": r["scenario"],
            "control_case": r["control_case"],
            "multi_turn": r.get("multi_turn", False),
            "turn_count": r.get("turn_count", 0),
            "expected_behavior": r["expected_behavior"],
            "defensive_action": r["defensive_action"],
            "breakthrough_detected": r["breakthrough_detected"],
            "requires_human_review": r["requires_human_review"],
            "signal_detected": r["signal_detected"],
            "erosion_detected": r.get("erosion_detected", False),
            "defensive_check_passed": r["defensive_check_passed"],
            "evaluation_summary": r["evaluation_summary"],
        }
        if r.get("turns_evaluated"):
            entry["turns_evaluated"] = r["turns_evaluated"]
        entries_out.append(entry)

    result_yaml = {
        "adversarial_result_metadata": {
            "run_id": "phase62d-mtbe-mvp",
            "assessment_mode": "adversarial_validation",
            "adversarial_playbook_type": "multi_turn_boundary_erosion",
            "confirmed_vulnerability": False,
            "formal_finding_allowed": False,
        },
        "per_entry_results": entries_out,
    }

    path = output_dir / "adversarial_result.yaml"
    with open(path, "w") as f:
        yaml.dump(result_yaml, f, allow_unicode=True, default_flow_style=False, sort_keys=False)
    print(f"  [OK] {path}")


def main():
    print("=" * 60)
    print("Phase 62D — Multi-turn Boundary Erosion MVP Execution")
    print("=" * 60)

    print("\nLoading playbook...")
    playbook = load_playbook(PLAYBOOK_PATH)
    meta = playbook.get("playbook_metadata", {})
    print(f"  Playbook: {PLAYBOOK_PATH.name}")
    print(f"  Type: {meta.get('adversarial_playbook_type', 'N/A')}")
    print(f"  Entries: {len(playbook.get('entries', []))}")

    print("\nSimulating multi-turn defense evaluation...")
    results = execute_playbook(playbook)
    print(f"  Entries evaluated: {len(results)}")
    print(f"  Multi-turn entries: {sum(1 for r in results if r.get('multi_turn'))}")
    print(f"  Breakthrough detected: {sum(1 for r in results if r['breakthrough_detected'])}")
    print(f"  Erosion detected: {sum(1 for r in results if r.get('erosion_detected'))}")
    print(f"  Human review required: {sum(1 for r in results if r['requires_human_review'])}")

    print("\nWriting execution results...")
    write_execution_results(results, OUTPUT_DIR)

    print("\nGenerating scorecard...")
    generate_scorecard(results, meta, OUTPUT_DIR)

    print("\nGenerating adversarial result...")
    generate_adversarial_result(results, OUTPUT_DIR)

    print(f"\n{'=' * 60}")
    print("Execution complete.")
    print(f"{'=' * 60}")


if __name__ == "__main__":
    main()
