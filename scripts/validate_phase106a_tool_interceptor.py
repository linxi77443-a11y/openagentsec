#!/usr/bin/env python3
"""Phase 106A — 动态工具调用参数注入与 MCP 结构化类型混淆拦截器 Validator.
...
def main():
    global checks_passed, checks_failed
    print("=" * 70)
    print("Phase 106A — 动态工具调用参数注入与 MCP 结构化类型混淆拦截器: Validator")
    print("Dynamic Tool-Call Parameter Injection & MCP Type Confusion Verification Suite")
    print("=" * 70)
    ...
    if playbook:
        ...
        check(meta.get("playbook_id") == "phase106a_dynamic_tool_interceptor_v1", "Playbook ID is valid")
        check(meta.get("phase") == "phase106a", "Phase is phase106a")
        check(meta.get("task_id") == "Phase-106A-TOOL-001", "Task ID is Phase-106A-TOOL-001")
        check(meta.get("module_id") == "DYNAMIC_TOOL_INTERCEPTOR", "Module ID is DYNAMIC_TOOL_INTERCEPTOR")
        ...
        check(meta.get("real_mcp_server_allowed") is False, "Playbook real_mcp_server_allowed is False")
        check(meta.get("real_tool_call_executed") is False, "Playbook real_tool_call_executed is False")
        ...
        for idx, e in enumerate(entries):
            ...
            check(bool(e.get("tool_name")), f"[{eid}] Tool name is specified")
            check(bool(e.get("mcp_method")), f"[{eid}] MCP method is specified")
            ...
            for field in ["synthetic_threat_signature", "synthetic_session", "synthetic_target_agent", "synthetic_payload", "tool_name", "target_endpoint"]:
                val = e.get(field, "")
                if val:
    ...
    if rc:
        ...
        check(rcfg.get("task_id") == "Phase-106A-TOOL-001", "Run config task_id is Phase-106A-TOOL-001")
        check(rcfg.get("module_id") == "DYNAMIC_TOOL_INTERCEPTOR", "Run config module_id is DYNAMIC_TOOL_INTERCEPTOR")
        ...
        params = rc.get("dynamic_tool_interceptor_parameters", {})
        check(params.get("schema_strict_validation_enabled") is True, "Schema strict validation enabled")
        check(params.get("mcp_type_enforcement_enabled") is True, "MCP type enforcement enabled")
        check(params.get("nested_escape_normalizer_enabled") is True, "Nested escape normalizer enabled")
        check(params.get("namespace_integrity_guard_enabled") is True, "Namespace integrity guard enabled")
        check(params.get("indirect_output_taint_tracking_enabled") is True, "Indirect output taint tracking enabled")
        check(params.get("additional_properties_strict_rejection_enabled") is True, "Additional properties strict rejection enabled")
        check(params.get("recursive_call_depth_circuit_breaker_enabled") is True, "Recursive call depth circuit breaker enabled")
        check(params.get("mcp_protocol_state_machine_verifier_enabled") is True, "MCP protocol state machine verifier enabled")
    ...
    if isinstance(exec_results, list):
        ...
        for r in exec_results:
            ...
            check(r.get("real_mcp_server_accessed") is False, f"[{eid}] real_mcp_server_accessed is False")
            check(r.get("real_tool_call_executed") is False, f"[{eid}] real_tool_call_executed is False")
            ...
            if not r.get("control_case"):
                ...
                check(r.get("tool_call_intercepted") is True, f"[{eid}] Attack case tool_call_intercepted is True")
                check(r.get("tool_call_completed_safely") is False, f"[{eid}] Attack case tool_call_completed_safely is False")
                ...
                check(r.get("tool_call_intercepted") is False, f"[{eid}] Control case tool_call_intercepted is False")
                check(r.get("tool_call_completed_safely") is True, f"[{eid}] Control case tool_call_completed_safely is True")
    ...
    if manifest:
        ...
        check(mmeta.get("task_id") == "Phase-106A-TOOL-001", "Manifest task_id is Phase-106A-TOOL-001")
        check(mmeta.get("module_id") == "DYNAMIC_TOOL_INTERCEPTOR", "Manifest module_id is DYNAMIC_TOOL_INTERCEPTOR")
    ...
    for rp in result_paths:
        ...
        r_data = yaml_load(rp)
        if r_data:
            check(r_data.get("task_id") == "Phase-106A-TOOL-001", f"[{rp.name}] task_id is Phase-106A-TOOL-001")
            check(r_data.get("module_id") == "DYNAMIC_TOOL_INTERCEPTOR", f"[{rp.name}] module_id is DYNAMIC_TOOL_INTERCEPTOR")
    ...
    for sp in scorecard_paths:
        ...
        if sc_data:
            ...
            check(smeta.get("task_id") == "Phase-106A-TOOL-001", f"[{sp.name}] Scorecard task_id is Phase-106A-TOOL-001")
            check(smeta.get("module_id") == "DYNAMIC_TOOL_INTERCEPTOR", f"[{sp.name}] Scorecard module_id is DYNAMIC_TOOL_INTERCEPTOR")
    ...
    if notes_path.exists():
        ...
        check("DYNAMIC_TOOL_INTERCEPTOR" in notes_text, "Notes mentions DYNAMIC_TOOL_INTERCEPTOR")
        check("Phase-106A-TOOL-001" in notes_text, "Notes mentions Phase-106A-TOOL-001")
    ...
    summary_path = ROOT / "phase106a_tool001_execution_summary.yaml"
    check(summary_path.exists(), f"Execution summary exists at {summary_path}")
    if summary_path.exists():
        s_data = yaml_load(summary_path)
        if s_data:
            check(s_data.get("task_id") == "Phase-106A-TOOL-001", "Summary task_id is Phase-106A-TOOL-001")
    ...
    if delivery_path.exists():
        ...
        if deliv:
            check(deliv[0].get if isinstance(deliv, list) else deliv.get("workplan_id") == "Phase-106A-TOOL-001", "delivery.json workplan_id is Phase-106A-TOOL-001")
    ...
    if checks_failed > 0:
        ...
        print("ALL CHECKS PASSED (100%) — Phase-106A-TOOL-001 Validated Successfully!")
        print("=" * 70)
        sys.exit(0)
