#!/usr/bin/env python3
"""Phase 62G — Role Boundary Bypass MVP Validation Script."""
import sys, json, yaml
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
PLAYBOOK_PATH = ROOT / "adversarial_playbooks/role_boundary_bypass_mvp/playbook.yaml"
RUN_CONFIG_PATH = ROOT / "capability_engine/configs/phase62g_rbb_mvp.yaml"
EXEC_DIR = ROOT / "executions/phase62g_rbb_mvp"
...
EXPECTED_ENTRY_IDS = [
    "RBB-001", "RBB-002", "RBB-003", "RBB-004", "RBB-005",
    "RBB-006", "RBB-007", "RBB-008", "RBB-009", "RBB-010",
    "RBB-011", "RBB-012", "RBB-901", "RBB-902",
]

EXPECTED_CATEGORIES = [
    "fake_role_declaration", "manager_authorization",
    "emergency_override", "temporary_proxy",
    "test_environment_claim", "audit_bypass_request",
    "role_name_fabrication", "cross_department_claim",
    "expired_role_claim", "multi_team_override", "control",
]

EXPECTED_BEHAVIORS = ["refuse", "normal_response"]
SIM_PARAMETER_PATTERNS = [
    "<SIM_ROLE_", "<SIM_NAME_", "<SIM_AUTH_TOKEN", "<SIM_APPROVAL_",
    "<SIM_INCIDENT_", "<SIM_EMERGENCY_", "<SIM_PROXY_", "<SIM_TEST_",
    "<SIM_COMPLIANCE_", "<SIM_PROJECT_", "<SIM_AUDIT_", "<SIM_EMPLOYEE_",
    "<SIM_TEAM_", "<SIM_SERVER>", "<SIM_TABLE_", "<SIM_TOOL_",
]
...
def validate_playbook():
    ...
    check("adversarial_playbook_type = role_boundary_bypass", meta.get("adversarial_playbook_type") == "role_boundary_bypass")
    ...
    check("attack_objective = role_escalation", meta.get("attack_objective") == "role_escalation")
...
def validate_run_config():
    ...
    check("run_id = phase62g-rbb-mvp", rc.get("run_id") == "phase62g-rbb-mvp")
    check("assessment_mode = adversarial_validation", rc.get("assessment_mode") == "adversarial_validation")
    check("adversarial_playbook_type = role_boundary_bypass", rc.get("adversarial_playbook_type") == "role_boundary_bypass")
...
def main():
    print("=" * 60)
    print("Phase 62G — RBB MVP Validation")
    print("=" * 60)
    ...
    if failed == 0:
        print(f"\nALL {total} CHECKS PASSED — RBB MVP is complete and consistent.")
        sys.exit(0)
    else:
