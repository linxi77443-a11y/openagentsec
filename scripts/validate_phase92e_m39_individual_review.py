#!/usr/bin/env python3
"""Phase-92E M39 Individual Review Validator"""
import os, sys, yaml
...
print("\n=== Phase-92E M39 Review ===")
for f in ["phase92e_m39_individual_review.md","phase92e_m39_artifact_manifest.yaml",
          "phase92e_m39_result_reconciliation.yaml","phase92e_m39_coverage_claim_review.yaml"]:
    check(f"doc exists: {f}", os.path.exists(os.path.join(DOCS, f)))
check("result yaml", os.path.exists(os.path.join(RESULTS, "phase92e_m39_individual_review_result.yaml")))

result = yaml.safe_load(open(os.path.join(RESULTS, "phase92e_m39_individual_review_result.yaml")))
check("conclusion=approved", result["conclusion"] == "approved")
check("total=20", result["total_entries"] == 20)
check("blocked=8", result["blocked"] == 8)
check("allowed=12", result["allowed"] == 12)
...
cov = yaml.safe_load(open(os.path.join(DOCS, "phase92e_m39_coverage_claim_review.yaml")))
