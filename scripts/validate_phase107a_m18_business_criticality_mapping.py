#!/usr/bin/env python3
"""Phase 107A — M18 Business Criticality Mapping MVP Validator.
...
def main():
    global checks_passed, checks_failed
    print("=" * 60)
    print("Phase 107A — M18 Business Criticality Mapping MVP Validation")
    print("=" * 60)
    ...
    # ================================================================
    print("\n1. Corpus")
    corpus_path = ROOT / "adversarial_playbooks/m18_business_criticality_mapping_mvp/playbook.yaml"
    ...
    if corpus:
        ...
        check("criticality_assessment_omission" in categories,
              "Corpus includes criticality_assessment_omission")
        check("business_impact_underestimation" in categories,
              "Corpus includes business_impact_underestimation")
        check("priority_sorting_error" in categories,
              "Corpus includes priority_sorting_error")
        check("critical_asset_marking_missing" in categories,
              "Corpus includes critical_asset_marking_missing")
        check("risk_transmission_path_ignored" in categories,
              "Corpus includes risk_transmission_path_ignored")
        check("remediation_priority_deviation" in categories,
              "Corpus includes remediation_priority_deviation")
        check("compliance_risk_underestimation" in categories,
              "Corpus includes compliance_risk_underestimation")
        check("recovery_time_target_inaccurate" in categories,
              "Corpus includes recovery_time_target_inaccurate")
        ...
        # Module metadata
        check(meta.get("module_id") == "M18", "Corpus module_id == M18")
        ...
        check(meta.get("real_itsm_allowed") is False,
              "Corpus real_itsm_allowed == false")
        check(meta.get("real_risk_assessment_platform_allowed") is False,
              "Corpus real_risk_assessment_platform_allowed == false")

    # ================================================================
    # 2. Run config
    # ================================================================
    print("\n2. Run config")
    rc_path = ROOT / "run_configs/phase107a_m18_business_criticality_mapping_run_config.yaml"
    rc = yaml_load(rc_path)
    check(rc is not None, "Run config YAML loaded")
    if rc:
        rcfg = rc.get("run_config", {})
        check(rcfg.get("phase") == "phase107a", "Run config phase == phase107a")
        check(rcfg.get("module_id") == "M18", "Run config module_id == M18")
        ...
        check(rcfg.get("real_business_system_allowed") is False,
              "Run config real_business_system_allowed == false")
        check(rcfg.get("real_cmdb_allowed") is False,
              "Run config real_cmdb_allowed == false")
        check(rcfg.get("real_itsm_allowed") is False,
              "Run config real_itsm_allowed == false")
        check(rcfg.get("real_risk_assessment_platform_allowed") is False,
              "Run config real_risk_assessment_platform_allowed == false")
    ...
    # ================================================================
    print("\n3. Execution results")
    exec_path = ROOT / "executions/phase107a_m18_mvp/execution_results.json"
    ...
    if exec_results:
        ...
        for r in exec_results:
            ...
            check(r.get("real_business_system_accessed") is False,
                  f"{rid} real_business_system_accessed == false")
            check(r.get("real_cmdb_accessed") is False,
                  f"{rid} real_cmdb_accessed == false")
            check(r.get("real_itsm_accessed") is False,
                  f"{rid} real_itsm_accessed == false")
            check(r.get("real_risk_assessment_platform_accessed") is False,
                  f"{rid} real_risk_assessment_platform_accessed == false")
    ...
    # ================================================================
    print("\n4. Result YAML")
    result_path = ROOT / "executions/phase107a_m18_mvp/m18_result.yaml"
    result_yaml = yaml_load(result_path)
    check(result_yaml is not None, "m18_result.yaml exists")
    if result_yaml:
        check(result_yaml.get("module_id") == "M18", "result module_id == M18")
        ...
        check("criticality_assessment_omission_count" in result_yaml,
              "result has criticality_assessment_omission_count")
        check("business_impact_underestimation_count" in result_yaml,
              "result has business_impact_underestimation_count")
        check("priority_sorting_error_count" in result_yaml,
              "result has priority_sorting_error_count")
        check("critical_asset_marking_missing_count" in result_yaml,
              "result has critical_asset_marking_missing_count")
        check("risk_transmission_path_ignored_count" in result_yaml,
              "result has risk_transmission_path_ignored_count")
        check("remediation_priority_deviation_count" in result_yaml,
              "result has remediation_priority_deviation_count")
        check("compliance_risk_underestimation_count" in result_yaml,
              "result has compliance_risk_underestimation_count")
        check("recovery_time_target_inaccurate_count" in result_yaml,
              "result has recovery_time_target_inaccurate_count")
        ...
        check(result_yaml.get("real_business_system_accessed") is False,
              "result real_business_system_accessed == false")
        check(result_yaml.get("real_cmdb_accessed") is False,
              "result real_cmdb_accessed == false")
        check(result_yaml.get("real_itsm_accessed") is False,
              "result real_itsm_accessed == false")
        check(result_yaml.get("real_risk_assessment_platform_accessed") is False,
              "result real_risk_assessment_platform_accessed == false")

    # ================================================================
    # 5. Scorecard
    # ================================================================
    print("\n5. Scorecard")
    scorecard_path = ROOT / "executions/phase107a_m18_mvp/capability_scorecard.yaml"
    ...
    if scorecard:
        ...
        check(sm.get("module_id") == "M18", "scorecard module_id == M18")
        ...
        check("criticality_assessment_omission_detected" in rs,
              "scorecard result_summary has criticality_assessment_omission_detected")
        check("business_impact_underestimation_detected" in rs,
              "scorecard result_summary has business_impact_underestimation_detected")
        check("priority_sorting_error_detected" in rs,
              "scorecard result_summary has priority_sorting_error_detected")
    ...
    # ================================================================
    print("\n6. Notes")
    notes_path = ROOT / "docs/phase107a_m18_business_criticality_mapping_mvp_notes.md"
    notes = file_exists(notes_path, "Notes")
    if notes:
        notes_text = notes_path.read_text()
        check("不连接真实业务系统" in notes_text or "no real business system" in notes_text.lower()
              or "real_business_system" in notes_text.lower(),
              "Notes state no real business system connection")
    ...
    if reg:
        ...
        m18 = next((m for m in modules if m.get("module_id") == "M18"), None)
        check(m18 is not None, "M18 exists in registry")
        if m18:
            check(m18.get("production_safety") == "out_of_scope",
                  "Registry M18 production_safety == out_of_scope")
            check(m18.get("confirmed_vulnerability_allowed") is False,
                  "Registry M18 confirmed_vulnerability_allowed == false")
            check(m18.get("formal_finding_allowed") is False,
                  "Registry M18 formal_finding_allowed == false")
            cov = m18.get("coverage", {})
            check(cov.get("coverage_status") == "mvp_complete",
                  "Registry M18 coverage_status == mvp_complete")
    ...
    print("\n" + "=" * 60)
    if checks_failed == 0:
        print("Phase 107A M18 Business Criticality Mapping validation: ALL CHECKS PASSED")
    else:
        print("Phase 107A M18 Business Criticality Mapping validation: FAILED")
