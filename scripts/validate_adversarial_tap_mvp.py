#!/usr/bin/env python3
"""Phase 62F — Tool Argument Pollution MVP Validation Script."""
import sys, json, yaml
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
PLAYBOOK_PATH = ROOT / "adversarial_playbooks/tool_argument_pollution_mvp/playbook.yaml"
RUN_CONFIG_PATH = ROOT / "capability_engine/configs/phase62f_tap_mvp.yaml"
EXEC_DIR = ROOT / "executions/phase62f_tap_mvp"
...
EXPECTED_ENTRY_IDS = [
    "TAP-001", "TAP-002", "TAP-003", "TAP-004", "TAP-005",
    "TAP-006", "TAP-007", "TAP-008", "TAP-009", "TAP-010",
    "TAP-011", "TAP-012", "TAP-901", "TAP-902",
]

EXPECTED_CATEGORIES = [
    "customer_id_injection", "tenant_id_pollution",
    "file_id_manipulation", "order_id_fabrication",
    "account_id_spoofing", "parameter_override",
    "sql_injection_in_arg", "path_traversal_in_arg",
    "bulk_parameter_pollution", "hidden_parameter_injection", "control",
]

EXPECTED_BEHAVIORS = ["refuse", "normal_response"]
SIM_PARAMETER_PATTERNS = [
    "<SIM_CUSTOMER_ID_", "<SIM_TENANT_ID_", "<SIM_FILE_ID_",
    "<SIM_ORDER_ID_", "<SIM_ACCOUNT_ID_", "<SIM_SQL_", "<SIM_PATH_",
    "<SIM_REPORT_ID", "<SIM_AUTH_TOKEN", "<SIM_TOOL_",
]
...
def validate_playbook():
    ...
    check("adversarial_playbook_type = tool_argument_pollution", meta.get("adversarial_playbook_type") == "tool_argument_pollution")
    ...
    for e in entries:
        ...
        # All adversarial prompts must use <SIM_...> placeholders
        prompt = e.get("prompt", "")
...
def validate_run_config():
    ...
    check("run_id = phase62f-tap-mvp", rc.get("run_id") == "phase62f-tap-mvp")
    check("assessment_mode = adversarial_validation", rc.get("assessment_mode") == "adversarial_validation")
    check("adversarial_playbook_type = tool_argument_pollution", rc.get("adversarial_playbook_type") == "tool_argument_pollution")
...
def main():
    print("=" * 60)
    print("Phase 62F — TAP MVP Validation")
    print("=" * 60)
    ...
    if failed == 0:
        print(f"\nALL {total} CHECKS PASSED — TAP MVP is complete and consistent.")
        sys.exit(0)
    else:
