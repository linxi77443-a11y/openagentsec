#!/usr/bin/env python3
"""Phase 62H — Service Account Abuse MVP Validation Script."""
import sys, json, yaml
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
PLAYBOOK_PATH = ROOT / "adversarial_playbooks/service_account_abuse_mvp/playbook.yaml"
RUN_CONFIG_PATH = ROOT / "capability_engine/configs/phase62h_saa_mvp.yaml"
EXEC_DIR = ROOT / "executions/phase62h_saa_mvp"
...
EXPECTED_ENTRY_IDS = [
    "SAA-001", "SAA-002", "SAA-003", "SAA-004", "SAA-005",
    "SAA-006", "SAA-007", "SAA-008", "SAA-009", "SAA-010",
    "SAA-011", "SAA-012", "SAA-901", "SAA-902",
]

EXPECTED_CATEGORIES = [
    "service_account_impersonation", "scope_boundary_violation",
    "cross_tenant_access", "cross_department_service_abuse",
    "action_type_escalation", "service_account_chaining",
    "background_task_fabrication", "scheduled_job_abuse",
    "read_only_to_write_escalation", "service_account_token_reuse", "control",
]

EXPECTED_BEHAVIORS = ["refuse", "normal_response"]
SIM_PARAMETER_PATTERNS = [
    "<SIM_SERVICE_ACCOUNT_", "<SIM_SA_TOKEN_", "<SIM_TENANT_ID_",
    "<SIM_S3_BUCKET_", "<SIM_DB_CLUSTER_", "<SIM_FTP_", "<SIM_TABLE_",
    "<SIM_SERVER>", "<SIM_AGENT_ID>", "<SIM_PIPELINE_ID>",
    "<SIM_CROSS_REGION_CODE>", "<SIM_CROSS_DEPT_PROJECT>",
    "<SIM_CLEANUP_CODE>", "<SIM_EMAIL_", "<SIM_MAINT_TICKET>",
    "<SIM_OPERATOR_ID>", "<SIM_BATCH_OVERRIDE>", "<SIM_CUSTOMER_ID>",
    "<SIM_PARTNER_ID>", "<SIM_INVOICE_ID>", "<SIM_MERGER_CASE>",
    "<SIM_TOOL_",
]
...
def validate_playbook():
    ...
    check("adversarial_playbook_type = service_account_abuse", meta.get("adversarial_playbook_type") == "service_account_abuse")
    ...
    check("attack_objective = service_account_abuse", meta.get("attack_objective") == "service_account_abuse")
...
def validate_run_config():
    ...
    check("run_id = phase62h-saa-mvp", rc.get("run_id") == "phase62h-saa-mvp")
    check("assessment_mode = adversarial_validation", rc.get("assessment_mode") == "adversarial_validation")
    check("adversarial_playbook_type = service_account_abuse", rc.get("adversarial_playbook_type") == "service_account_abuse")
...
def main():
    print("=" * 60)
    print("Phase 62H — SAA MVP Validation")
    print("=" * 60)
    ...
    if failed == 0:
        print(f"\nALL {total} CHECKS PASSED — SAA MVP is complete and consistent.")
        sys.exit(0)
    else:
