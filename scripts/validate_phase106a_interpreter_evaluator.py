#!/usr/bin/env python3
"""Phase 106A — 代码解释器沙箱越权与环境变量探测防御评测器 Validator.
...
def main():
    global checks_passed, checks_failed
    print("=" * 70)
    print("Phase 106A — 代码解释器沙箱越权与环境变量探测防御评测器: Validator")
    print("Code Interpreter Sandbox Escape & Env Probe Verification Suite")
    print("=" * 70)
    ...
    if playbook:
        ...
        check(meta.get("playbook_id") == "phase106a_interpreter_sandbox_evaluator_v1", "Playbook ID is valid")
        check(meta.get("phase") == "phase106a", "Phase is phase106a")
        check(meta.get("task_id") == "Phase-106A-INTERPRETER-002", "Task ID is Phase-106A-INTERPRETER-002")
        check(meta.get("module_id") == "CODE_INTERPRETER_SANDBOX_EVALUATOR", "Module ID is CODE_INTERPRETER_SANDBOX_EVALUATOR")
        ...
        check(meta.get("real_code_interpreter_host_execution_allowed") is False, "Playbook real_code_interpreter_host_execution_allowed is False")
        check(meta.get("real_sandbox_escape_allowed") is False, "Playbook real_sandbox_escape_allowed is False")
        check(meta.get("real_env_access_allowed") is False, "Playbook real_env_access_allowed is False")
        ...
        for idx, e in enumerate(entries):
            ...
            check(bool(e.get("interpreter_runtime")), f"[{eid}] Interpreter runtime is specified")
            ...
            for field in ["synthetic_threat_signature", "synthetic_session", "synthetic_target_agent", "synthetic_payload", "interpreter_runtime", "target_endpoint"]:
                val = e.get(field, "")
                if val:
    ...
    if rc:
        ...
        check(rcfg.get("phase") == "phase106a", "Run config phase is phase106a")
        check(rcfg.get("task_id") == "Phase-106A-INTERPRETER-002", "Run config task_id is Phase-106A-INTERPRETER-002")
        check(rcfg.get("module_id") == "CODE_INTERPRETER_SANDBOX_EVALUATOR", "Run config module_id is CODE_INTERPRETER_SANDBOX_EVALUATOR")
        ...
        params = rc.get("interpreter_evaluator_parameters", {})
        check(params.get("ast_dunder_reflection_guard_enabled") is True, "AST dunder reflection guard enabled")
        check(params.get("env_var_sandbox_isolation_enabled") is True, "Env var sandbox isolation enabled")
        check(params.get("native_module_firewall_enabled") is True, "Native module firewall enabled")
        check(params.get("subprocess_process_tree_filter_enabled") is True, "Subprocess process tree filter enabled")
        check(params.get("network_namespace_isolation_enabled") is True, "Network namespace isolation enabled")
        check(params.get("chroot_virtual_fs_guard_enabled") is True, "Chroot virtual fs guard enabled")
        check(params.get("dynamic_eval_deobfuscator_enabled") is True, "Dynamic eval deobfuscator enabled")
        check(params.get("runtime_resource_quota_governor_enabled") is True, "Runtime resource quota governor enabled")
    ...
    if isinstance(exec_results, list):
        ...
        for r in exec_results:
            ...
            check(r.get("real_code_interpreter_host_executed") is False, f"[{eid}] real_code_interpreter_host_executed is False")
            check(r.get("real_sandbox_escape_attempted") is False, f"[{eid}] real_sandbox_escape_attempted is False")
            ...
            if not r.get("control_case"):
                ...
                check(r.get("code_execution_intercepted") is True, f"[{eid}] Attack case code_execution_intercepted is True")
                check(r.get("code_execution_completed_safely") is False, f"[{eid}] Attack case code_execution_completed_safely is False")
                ...
                check(r.get("code_execution_intercepted") is False, f"[{eid}] Control case code_execution_intercepted is False")
                check(r.get("code_execution_completed_safely") is True, f"[{eid}] Control case code_execution_completed_safely is True")
    ...
    if manifest:
        ...
        check(mmeta.get("task_id") == "Phase-106A-INTERPRETER-002", "Manifest task_id is Phase-106A-INTERPRETER-002")
        check(mmeta.get("module_id") == "CODE_INTERPRETER_SANDBOX_EVALUATOR", "Manifest module_id is CODE_INTERPRETER_SANDBOX_EVALUATOR")
    ...
    for rp in result_paths:
        ...
        r_data = yaml_load(rp)
        if r_data:
            check(r_data.get("task_id") == "Phase-106A-INTERPRETER-002", f"[{rp.name}] task_id is Phase-106A-INTERPRETER-002")
            check(r_data.get("module_id") == "CODE_INTERPRETER_SANDBOX_EVALUATOR", f"[{rp.name}] module_id is CODE_INTERPRETER_SANDBOX_EVALUATOR")
    ...
    for sp in scorecard_paths:
        ...
        if sc_data:
            ...
            check(smeta.get("task_id") == "Phase-106A-INTERPRETER-002", f"[{sp.name}] Scorecard task_id is Phase-106A-INTERPRETER-002")
            check(smeta.get("module_id") == "CODE_INTERPRETER_SANDBOX_EVALUATOR", f"[{sp.name}] Scorecard module_id is CODE_INTERPRETER_SANDBOX_EVALUATOR")
    ...
    if notes_path.exists():
        ...
        check("CODE_INTERPRETER_SANDBOX_EVALUATOR" in notes_text, "Notes mentions CODE_INTERPRETER_SANDBOX_EVALUATOR")
        check("Phase-106A-INTERPRETER-002" in notes_text, "Notes mentions Phase-106A-INTERPRETER-002")
    ...
    if summary_path.exists():
        ...
        if s_data:
            check(s_data.get("task_id") == "Phase-106A-INTERPRETER-002", "Summary task_id is Phase-106A-INTERPRETER-002")
    ...
    if delivery_path.exists():
        ...
        if deliv:
            check(deliv[0].get if isinstance(deliv, list) else deliv.get("workplan_id") == "Phase-106A-INTERPRETER-002", "delivery.json workplan_id is Phase-106A-INTERPRETER-002")
    ...
    if checks_failed > 0:
        ...
        print("ALL CHECKS PASSED (100%) — Phase-106A-INTERPRETER-002 Validated Successfully!")
        print("=" * 70)
        sys.exit(0)
