#!/usr/bin/env python3
"""Phase 62D — Multi-turn Boundary Erosion MVP Validation Script."""
import sys, json, yaml
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
PLAYBOOK_PATH = ROOT / "adversarial_playbooks/multi_turn_boundary_erosion_mvp/playbook.yaml"
RUN_CONFIG_PATH = ROOT / "capability_engine/configs/phase62d_mtbe_mvp.yaml"
EXEC_DIR = ROOT / "executions/phase62d_mtbe_mvp"
...
EXPECTED_ENTRY_IDS = [
    "MTBE-001", "MTBE-002", "MTBE-003", "MTBE-004", "MTBE-005",
    "MTBE-006", "MTBE-007", "MTBE-008", "MTBE-009", "MTBE-010",
    "MTBE-011", "MTBE-012", "MTBE-901", "MTBE-902",
]

EXPECTED_CATEGORIES = [
    "gradual_role_escalation", "contextual_boundary_testing",
    "cumulative_data_extraction", "trust_building_exploitation",
    "distraction_and_switch", "false_compliance_trap",
    "emotional_manipulation", "multi_turn_persistent",
    "assertion_escalation", "tool_access_erosion", "control",
]

EXPECTED_BEHAVIORS = ["refuse", "block_tool_call", "request_approval", "normal_response"]
...
def validate_playbook():
    ...
    check("adversarial_playbook_type = multi_turn_boundary_erosion", meta.get("adversarial_playbook_type") == "multi_turn_boundary_erosion")
    ...
    check("attack_objective = unauthorized_access", meta.get("attack_objective") == "unauthorized_access")
    ...
    for e in entries:
        ...
        turns = e.get("turns", [])
        check(f"{eid}: has multi-turn turns", len(turns) >= 2, f"{len(turns)} turns")
        for t in turns:
            check(f"{eid}: turn {t['turn_number']} has role", "role" in t)
            check(f"{eid}: turn {t['turn_number']} has content", bool(t.get("content")))
...
def validate_run_config():
    ...
    check("run_id = phase62d-mtbe-mvp", rc.get("run_id") == "phase62d-mtbe-mvp")
    check("assessment_mode = adversarial_validation", rc.get("assessment_mode") == "adversarial_validation")
    check("adversarial_playbook_type = multi_turn_boundary_erosion", rc.get("adversarial_playbook_type") == "multi_turn_boundary_erosion")
...
def validate_execution_results():
    ...
    for r in results:
        ...
        check(f"{eid}: multi_turn flag", r.get("multi_turn") is True)
        check(f"{eid}: turn_count >= 2", r.get("turn_count", 0) >= 2)
...
def validate_scorecard():
    ...
    check("multi_turn_entries > 0", meta.get("multi_turn_entries", 0) > 0)
...
def main():
    print("=" * 60)
    print("Phase 62D — MTBE MVP Validation")
    print("=" * 60)
    ...
    if failed == 0:
        print(f"\nALL {total} CHECKS PASSED — MTBE MVP is complete and consistent.")
        sys.exit(0)
    else:
